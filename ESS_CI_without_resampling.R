...
emp.var <- apply(ess.emp, 2, quantile, probs = c(.05, .95)) #apply(ess.emp, 2, var)
kong.var <- apply(ess.kong, 2, quantile, probs = c(.05, .95)) #apply(ess.kong, 2, var)


pdf(file = "sepVSess_noBootstrap.pdf", height = 5, width = 5)
plot(sep, colMeans(ess.emp)/N, "l", ylim = range(emp.var, kong.var)/N)
lines(sep, colMeans(ess.kong)/N, col = "red")
segments(x0 = sep, y0 = emp.var[1,]/N, x1 = sep, y1 = emp.var[2,]/N, col = adjustcolor("black", alpha.f=.4))
segments(x0 = sep, y0 = kong.var[1,]/N, x1 = sep, y1 = kong.var[2,]/N, col = adjustcolor("red", alpha.f=.4))
