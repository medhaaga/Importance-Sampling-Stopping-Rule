...
for(i in 1:3)
{
  ...
  pdf(file = paste("Out/biNorm-ellipse_setting", i, ".pdf", sep = ""), height=5, width=5)
  ...
  legend("topleft", legend = c("Target variance", "Proposal variance", "UIS variance", "SNIS variance"), bty = 'n', cex=.8, col = c("black", "blue", "red", "orange"), lty=c(2,2,1,1), lwd=2)
  dev.off()
}
...
for (t in 1:3)
{
  ...
  pdf(file = paste("Out/biNormal-trueESSvsRho_p2_setting", t, ".pdf", sep = ""), width = 5, height = 5)
  plot(rhos, true.ess.uis[1,], type = "l", col = "red", lwd=2, ylim = range(true.ess.uis, true.ess.snis, 1.2), ylab = "True ESS/n", xlab = expression(rho))
  ...
  if(t==1)
    legend("topright", legend = c("UIS", "SNIS"), bty = 'n', cex=.8, col = c("red", "orange"), lwd=2) else
      legend("topleft", legend = c("UIS", "SNIS"), bty = 'n', cex=.8, col = c("red", "orange"), lwd=2)
...
for (j in 1:2)
{
  for (a in 1:3)
  {
    ...
    pdf(file = paste("Out/multinorm-sqeVSiter_p", p, "_setting", a, ".pdf", sep = ""), width = 5, height = 5)
    
    plot(log(all_ESS1[[1]]$emp_uis[,1]), SE1_uis, pch=1, col = "blue", main = paste("p = ", p), xlim = log(range(truth.snis, truth.uis, all_ESS1[[1]]$emp_uis[,1],  all_ESS1[[1]]$emp_snis[,1], all_ESS3[[1]]$emp_uis[,1],  all_ESS3[[1]]$emp_snis[,1])), ylim = range(ymax, SE1_uis, SE2_uis, SE3_uis, SE1_snis, SE2_snis, SE3_snis), xlab = "Number of Samples", ylab = "Squared error", xaxt="n")
    ...
    legend("topright", border = NA, bty = 'n', legend = c(expression(paste(epsilon, "=0.06")), expression(paste(epsilon, "=0.04")), expression(paste(epsilon, "=0.02"))), col = c("blue", "green", "orange"), pch = 19, cex = 1.2)
    legend("topright", inset = c(.25, 0.02), bty = 'n', legend = c("UIS", "SNIS"), pch = c(1,2))
    dev.off()
  }
