...
ess.kong <- matrix(0, nrow = reps, ncol = length(prop_shape))
ess.emp <- matrix(0, nrow = reps, ncol = length(prop_shape))
...
plot(prop_shape, colMeans(ess.emp/N), type = "l", col = "blue", ylab = "ESS/N", xlab = "Proposal shape parameter", ylim = range(ess.emp, ess.truth, ess.kong)/N)
lines(prop_shape, colMeans(ess.kong/N), col = "orange")
